...
| `creds reference list <group> <credential> [--depth <1..16>] [--json]`                                                                | Available in the built package | Walks a credential's outbound references on this device to a bounded depth, labelling every archived target, missing target, cycle, and repeat rather than dropping it. Defaults to one level; nothing is sent anywhere.                                                                                                                                                                                       |
| `creds reference add <group> <credential> <field> <target> [opts]`                                                                    | Available in the built package | Points an item-reference field at another credential, writing the field value and the relation in the same mutation. Refuses a self-reference, and refuses a cycle until `--allow-cycle` records the loop as intended. `--target-group` reaches a target in another group; `--if-revision` guards the write; re-adding an existing reference changes nothing.                                                  |
| `creds reference remove <group> <credential> <field> [--target <q>] [opts]`                                                           | Available in the built package | Drops one reference from a field, retiring the relation only when no remaining field binds that target. `--target` is required when the field holds more than one reference, and a target that no longer resolves is still removable.                                                                                                                                                                          |
...
| `creds purge preview [--group <q>] [--credential <q>] [--older-than <days>] [--category <c...>] [--json]`                             | Available in the built package | Reports what a purge would destroy on this device without destroying anything, giving every unit a disposition: purgeable, retained inside the requested window, retained by the vault's own tombstone clock, or unsupported locally. It also names the categories this client cannot enumerate at all, so a zero count is never read as an empty vault. Nothing is written and nothing is sent anywhere.      |
| `creds purge fields <group> <credential> --force [--field <q>] [--reason <r...>] [--older-than <days>] [--if-revision <n>]`           | Available in the built package | Permanently destroys a credential's archived field values as one revision-bound mutation, retiring only the relations the purge itself unbinds. Requires `--force`, because neither `field restore` nor `history restore` can bring an archived value back. Attachments the purge leaves unreferenced are reported rather than unlinked, so the operator keeps the only handle on the blob.                    |
| `creds purge notes <group> [--credential <q>] --force [--note <id>] [--older-than <days>] [--if-revision <n>]`                        | Available in the built package | Permanently destroys the archived notes of a group or of one credential, validating the surviving collection on each removal. Requires `--force`. An active note is never selected, so a purge cannot be confused with `note archive`, and `--note` resolves by ID only because a title is not a safe handle for an irreversible write.                                                                        |
