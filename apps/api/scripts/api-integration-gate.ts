...
interface ExecuteGateOptions extends GateOptions {
...
type RunChild = (invocation: BoundedChildInvocation) => Promise<BoundedChildResult>;

export async function discoverApiIntegrationFiles(
  workspaceRoot: string,
): Promise<string[]> {
  const integrationRoot = join(workspaceRoot, 'apps', 'api', 'integration');
...
export async function runApiIntegrationGate(
  options: GateOptions = {},
): Promise<GateSummary> {
  const environment = options.environment ?? process.env;
  requireMongoEnvironment(environment);
  const workspaceRoot = options.workspaceRoot ?? defaultWorkspaceRoot();
  const integrationFiles = await discoverApiIntegrationFiles(workspaceRoot);
  ...
  const invocation: BoundedChildInvocation = {
    ...
    shell: false,
    timeoutMs: options.timeoutMs ?? API_INTEGRATION_TIMEOUT_MS,
    workspaceRoot,
  };
...
export async function executeApiIntegrationGate(
  options: ExecuteGateOptions = {},
): Promise<number> {
  const writeError = options.stderr ?? console.error;
  const writeOutput = options.stdout ?? console.log;
  try {
    const summary = await runApiIntegrationGate(options);
    writeOutput(
      `API integration tests passed (${String(summary.files)} files, ${String(summary.tests)} tests).`,
    );
    return 0;
  } catch (error) {
    writeError(error instanceof Error ? error.message : genericGateError);
    return 1;
  }
}
...
if (
  ...
  process.exitCode = await executeApiIntegrationGate();
}
