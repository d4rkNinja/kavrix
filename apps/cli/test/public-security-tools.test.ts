...
describe('public secret-output boundary', () => {
  ...
  it('never reads a seed from argv/environment or reveals canaries in errors/completion', async () => {
    ...
    expect(completion.stdout).toContain(
      'version generate totp key init connect recover unlock lock status template group credential field note attachment show copy reveal get sync backup device completion',
    );
