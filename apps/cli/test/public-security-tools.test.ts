...
describe('public secret-output boundary', () => {
  ...
  it('never reads a seed from argv/environment or reveals canaries in errors/completion', async () => {
    ...
    expect(completion.stdout).toContain(
      'version generate totp key init connect recover unlock lock status template group credential field note attachment history audit show copy reveal get set update run sync backup device completion',
    );
