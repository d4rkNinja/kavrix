...
Interactive `kavrix init` opens Ink onboarding (`mountOnboardingApp`) by default when stdin+stdout+stderr are TTYs. It dispatches real `create-file-profile` / `create-mongodb-profile` via `createCliTuiBackend` (no mocks), including recovery-kit passphrase + path collection and real `db recovery create` + `verify` (same authenticated outcome as classic `--no-tui` guided local onboarding). Presentation flags: `--ascii`, `--color` / `--no-color`, `--no-splash` (also `KAVRIX_TUI_NO_SPLASH`).
...
| ------------------------------------------------------------------------------------------------------------------- | ------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. TTY default → Ink TUI (file + mongodb paths + recovery kit)                                                      | PASS   | Router unit walk (owner + recovery passphrase/confirm + `~/.kavrix` recovery path) + session create wires `db recovery create/verify`; `shouldRunInitTuiOnboarding` + init action invokes `runInitTuiOnboarding` (Ink mount needs real TTY; not driven headless) |
...
| 10. TUI recovery kit (masked passphrase confirm + secure path; create+verify; failure retains vault with guidance)  | PASS   | Router recovery steps + `tui-session` create-file-profile recovery frames; retained-profile on recovery failure after vault                                                                                                                                      |

**Honest limits:** full Ink keystroke drive of `kavrix init` still requires a real TTY (headless box covers routing/validation/backend; interactive paint is covered by unit router + manual/desktop demos). Mid-create SIGKILL can still leave orphan data/key files even after profile remove. If recovery create/verify fails after the vault exists, the profile is retained with explicit cleanup/verify guidance (not silently half-deleted).
